...
def lookup_online_dictionary(word):
    """Tra từ điển online (dictionaryapi.dev)"""
    try:
        import requests
...
def concat_audio_mp3(files, output):
    ...
    cmd = [
        ...
        "-ar", "44100",
        "-b:a", "128k",
        output   # ← output vẫn là .mp3 nhưng thêm -f mp3 để ép format
    ]
    # Thêm -f mp3 trước output
    cmd.insert(-1, "-f")
    cmd.insert(-1, "mp3")
...
'''
    ...
    def render_segment(self, word, meaning, example, vid_idx, seg_idx):
        ...
        if bg_file and os.path.exists(bg_file):
            ...
            # ✅ Thêm logo nếu được bật
            if LOGO_ENABLED and LOGO_PATH and os.path.exists(LOGO_PATH):
                logo_f, overlay_f = get_logo_overlay_filter(
                    LOGO_PATH, LOGO_SIZE, LOGO_POS, LOGO_ALPHA)

                if logo_f and overlay_f:
                    # Dùng complex filtergraph
                    complex_filter = (
                        f"[0:v]scale={VIDEO_W}:{VIDEO_H}:force_original_aspect_ratio=increase,"
                        f"crop={VIDEO_W}:{VIDEO_H},format=rgba,"
                        + vf_text.replace(",", ",")
                        + f"[bg];"
                        + logo_f + ";"
                        + f"[bg][logo]overlay={LOGO_POS}[out]"
                    )
                    cmd = (
                        [FFMPEG_PATH, "-y"]
                        + loop_flag
                    ...
                    cmd = _build_normal_cmd(...)  # fallback
            else:
                # Không có logo - giữ nguyên command cũ
                cmd = (
                    [FFMPEG_PATH, "-y"]
                    + loop_flag
                    + ["-stream_loop", "-1", "-i", bg_file]
...
class App:
    ...
    def _build_tab_voice(self, parent):
        ...
        for text, val in [
            ...
            tk.Radiobutton(parent, text=text,
                        variable=self.voice_mode_var,
                        value=val,
                        bg="#f0f4f8").pack(anchor="w", padx=20)

        ttk.Button(parent, text="💾 Lưu",
                command=self.save_settings).pack(pady=20)

    def _build_tab_background(self, parent):
        tk.Label(parent, text="🎥 Background Source",
                font=("Segoe UI", 14, "bold"),
                bg="#f0f4f8").pack(pady=10)
    ...
    def _build_tab_logo(self, parent):
        ...
        tk.Checkbutton(parent, text="Bật logo",
                    variable=self.logo_enabled_var,
                    bg="#f0f4f8").pack(anchor="w", padx=20)

        self.logo_path_var = tk.StringVar(value=LOGO_PATH)

        tk.Entry(parent, textvariable=self.logo_path_var).pack(fill="x", padx=20, pady=5)

        ttk.Button(parent, text="📂 Chọn logo",
                command=self._browse_logo).pack(pady=5)

        self.logo_size_var = tk.IntVar(value=LOGO_SIZE)
        tk.Label(parent, text="Size").pack()
        tk.Spinbox(parent, from_=30, to=300,
                textvariable=self.logo_size_var).pack()
    ...
    def _build_tab_system(self, parent):
        ...
        ttk.Button(parent, text="✅ Check FFmpeg",
                command=lambda: messagebox.showinfo("FFmpeg", check_ffmpeg()[1])
                ).pack(pady=5)

        ttk.Button(parent, text="📁 Open Output",
                command=self.open_output).pack(pady=5)
    ...
    def _build_tab_settings(self, parent):
        tk.Label(parent, text="⚙️ Cài đặt Voice",
                font=("Segoe UI", 12, "bold"),
                bg="#f0f4f8").pack(anchor="w", padx=12, pady=10)
        ...
        options = [
            ...
            ("⚡ Hybrid (khuyên dùng: Word+Example = Eleven, Meaning = Edge)", "hybrid")
        ]
        ...
        # ---- BG SOURCE ----
        tk.Label(parent, text="🎬 Nguồn Video Background",
                font=("Segoe UI", 11, "bold"),
                bg="#f0f4f8").pack(anchor="w", padx=12, pady=(16, 4))
        ...
        # API Keys cho BG
        key_frame2 = tk.LabelFrame(parent, text="API Keys (Background)", bg="#f0f4f8")
        key_frame2.pack(fill="x", padx=12, pady=8)

        tk.Label(key_frame2, text="Pexels API Key:", bg="#f0f4f8").grid(
            row=0, column=0, sticky="w", padx=10, pady=4)
        self.pexels_key_var = tk.StringVar(value=PEXELS_API_KEY)
        tk.Entry(key_frame2, textvariable=self.pexels_key_var,
                width=55, show="*").grid(row=0, column=1, padx=6, pady=4)

        tk.Label(key_frame2, text="Pixabay API Key:", bg="#f0f4f8").grid(
            row=1, column=0, sticky="w", padx=10, pady=4)
        self.pixabay_key_var = tk.StringVar(value=PIXABAY_API_KEY)
        tk.Entry(key_frame2, textvariable=self.pixabay_key_var,
                width=55, show="*").grid(row=1, column=1, padx=6, pady=4)

        tk.Label(key_frame2, text="giphy API Key:", bg="#f0f4f8").grid(
            row=2, column=0, sticky="w", padx=10, pady=4)
        self.giphy_key_var = tk.StringVar(value=GIPHY_API_KEY)
        tk.Entry(key_frame2, textvariable=self.giphy_key_var,
                width=55, show="*").grid(row=2, column=1, padx=6, pady=4)    

        # Button save
        ttk.Button(parent, text="💾 Lưu cài đặt",
                command=self.save_settings).pack(pady=10)

        # Info box
        self.settings_log = scrolledtext.ScrolledText(
            parent, height=10, font=("Consolas", 10),
            state="disabled", bg="#0f172a", fg="#94a3b8"
        )
        self.settings_log.pack(fill="both", expand=True, padx=12, pady=10)
        ...
        tk.Label(logo_frame, text="Kích thước (px):", bg="#f0f4f8").grid(
            row=2, column=0, sticky="w", padx=10, pady=4)
        self.logo_size_var = tk.IntVar(value=80)
        tk.Spinbox(logo_frame, from_=30, to=300, textvariable=self.logo_size_var,
                width=8).grid(row=2, column=1, sticky="w", padx=6, pady=4)

        # Vị trí logo
        tk.Label(logo_frame, text="Vị trí:", bg="#f0f4f8").grid(
            row=3, column=0, sticky="w", padx=10, pady=4)
        self.logo_pos_var = tk.StringVar(value="top-left")
        ...
        self.logo_alpha_var = tk.DoubleVar(value=0.85)
        ...
        ttk.Button(parent, text="📚 Quản lý từ điển",
           command=self.open_dict_manager).pack(pady=5)
    ...
    def _browse_logo(self):
        ...
        if path:
            ...
            self._settings_log(f"✅ Logo: {path}")
    ...
    def save_settings(self):
        ...
        self._settings_log(f"✅ Voice mode: {VOICE_MODE}")
        self._settings_log(f"✅ BG Source: {BG_SOURCE}")
        self._settings_log(f"✅ Logo: {'Bật' if LOGO_ENABLED else 'Tắt'} | {LOGO_PATH}")

        messagebox.showinfo("Đã lưu", f"Voice: {VOICE_MODE}\nBackground: {BG_SOURCE}\nLogo: {'Bật' if LOGO_ENABLED else 'Tắt'}")

    def _settings_log(self, msg):
        self.root.after(0, self.__settings_log_safe, msg)

    def __settings_log_safe(self, msg):
        self.settings_log.config(state="normal")
        self.settings_log.insert("end", msg + "\n")
        self.settings_log.see("end")
        self.settings_log.config(state="disabled")

    def open_dict_manager(self):
        import subprocess
        subprocess.Popen(["python", os.path.join(BASE_DIR, "dict_manager_ui.py")])
