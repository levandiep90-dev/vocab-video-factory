...
# Màu gradient theo cặp (start_color, end_color) dạng hex
GRADIENT_THEMES = [
    ("#0f0c29", "#302b63"),
    ("#1a1a2e", "#16213e"),
    ("#0d0d0d", "#1a1a2e"),
...
# Từ điển offline nhỏ (fallback nếu không có internet)
MINI_DICT = {
...
def lookup_word(word):
    """
    Tra từ điển offline.
    Trả về (meaning, example) hoặc fallback nếu không tìm thấy.
    """
    ...
    # Fallback thông minh
    meaning = f"[{word}]"
    example = f"This is {word}."
    return meaning, example


def auto_split_words(word_list, group_size=6):
    """
    Tự động chia danh sách từ thành các nhóm group_size từ.
    Nhóm cuối nếu thiếu sẽ được bỏ qua (hoặc pad).
    """
    groups = []
    for i in range(0, len(word_list), group_size):
        ...
        if len(chunk) == group_size:
            ...
            # Pad bằng từ cuối cùng nếu thiếu
...
def build_row_from_words(word_group):
    """
    Từ danh sách 6 từ, tra từ điển và tạo row 18 cột.
    """
    row = []
...
'''
    ...
    def render_segment(self, word, meaning, example, vid_idx, seg_idx):
        """
        Tạo 1 clip cho 1 từ với layout đẹp hơn:
        - Dòng 1: Từ tiếng Anh (to, vàng)
        - Dòng 2: Nghĩa tiếng Việt (trắng)
        - Dòng 3: Ví dụ (xám nhạt, nhỏ hơn)
        """
        ensure_output_dir()
...
class App:
    ...
    def _build_tab_quick_input(self, parent):
        ...
        self.word_input = scrolledtext.ScrolledText(parent, height=14, font=("Consolas", 11))
        self.word_input.pack(fill="both", expand=True, padx=12, pady=8)

        # Điền mẫu
        sample_words = "\n".join([
            "run", "walk", "jump", "climb", "stand", "sit",
    ...
    def _build_tab_csv(self, parent):
        ...
        tk.Label(parent, text="📋 Nội dung CSV (18 cột mỗi dòng):",
                 font=("Segoe UI", 10, "bold"), bg="#f0f4f8").pack(anchor="w", padx=12, pady=(8, 2))

        self.csv_view = scrolledtext.ScrolledText(parent, height=20, font=("Consolas", 9),                                        state="disabled", bg="#1e293b", fg="#e2e8f0")
    ...
    def _build_tab_render(self, parent):
        ...
        self.info_label = tk.Label(info_frame, text="Chưa có dữ liệu. Vui lòng load CSV hoặc nhập từ.",
                                    fg="#6b7280", bg="#f0f4f8")
    ...
    def _get_groups(self):
        ...
        if not words:
            return []
        return auto_split_words(words, group_size=6)
    ...
    def preview_groups(self):
        ...
        for i, group in enumerate(groups, 1):
            row = build_row_from_words(group)
            self.preview_box.insert("end", f"--- Video {i} ---\n")
            for j in range(6):
                word = row[j * 3]
                meaning = row[j * 3 + 1]
                example = row[j * 3 + 2]
                self.preview_box.insert("end", f"  [{j+1}] {word} | {meaning} | {example}\n")
            self.preview_box.insert("end", "\n")

        self.preview_box.config(state="disabled")
        self.safe_log(f"👁️ Xem trước: {len(groups)} nhóm từ ({len(self._get_word_list())} từ)")
    ...
    def export_to_csv(self):
        ...
        try:
            ...
            messagebox.showinfo("Thành công", f"Đã xuất {len(rows)} dòng vào:\n{save_path}")
        except Exception as e:
    ...
    def export_and_render(self):
        ...
        save_path = filedialog.asksaveasfilename(
            title="Lưu file CSV trước khi render",
            defaultextension=".csv",
            filetypes=[("CSV files", "*.csv")],
            initialfile="data.csv"
        )
    ...
    def build_exe(self):
        ...
        def run():
            ...
            if result.returncode == 0:
                ...
                self._exe_log(f"✅ Build thành công!")
                self._exe_log(f"📁 File EXE: {exe_path}")
                self.root.after(0, lambda: messagebox.showinfo(
                    "Build thành công",
                    f"File EXE đã được tạo tại:\n{exe_path}\n\n"
                    "Lưu ý: Copy ffmpeg.exe vào cùng thư mục với file EXE trước khi dùng!"
                ))
            else:
                self._exe_log(f"❌ Build thất bại!")
